...
export default function UPage() {
  ...
  const stancePrompts = {
    self: 'あなたは投稿者本人です。自分が作成したSNS記事を紹介する立場で、要約とSNS投稿文を作成してください。過度な自画自賛は避けつつ、背景やねらい、見どころを簡潔に添えてください。',
    others: 'あなたは第三者として、他人のSNS記事を自分のフォロワーに紹介します。著者へのリスペクトを示し、出典・引用であることを明確にしつつ、紹介者としての簡単な一言コメントを添えてください。',
    third: 'あなたは中立の紹介者です。第三者の記事を客観的に要約し、価値やポイント、読むべき理由を端的に伝えてください。主観を抑え、出典明記を前提にしてください。'
  } as const;

  // ===== 画像 → SNS =====
  const [imageFile, setImageFile] = useState<File | null>(null);
  const [isGenerating, setIsGenerating] = useState(false);
  const [imageNote, setImageNote] = useState(''); // 補足説明
  ...
  return (
    <main style={pageStyle}>
      <h2 style={{ fontSize: 20, fontWeight: 800, marginBottom: 12, color: colors.ink }}>ユーザーページ</h2>
              ...
              <div>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                  {urlHashtags.map((h, i) => (
                    <span
                      key={i}
                      style={{ border: '1px solid #eee', borderRadius: 999, padding: '6px 10px', background: '#fff' }}
                    >
                      {h}
                    </span>
                  ))}
                ...
                <div style={{ marginTop: 8 }}>
                  <button style={btnGhost} onClick={() => copy(urlHashtags.join(' '))}>
                    すべてコピー
                  </button>
                </div>
              </div>
      ...
      <div style={{ ...panel, marginBottom: 16 }}>
        ...
        <div style={{ display: 'grid', gap: 8, marginBottom: 12 }}>
          <label style={labelStyle}>補足説明（どんな写真か、状況など 3行程度）</label>
          <textarea
            style={{ ...inputStyle, height: 80, resize: 'vertical', whiteSpace: 'pre-wrap' }}
      ...
      <div style={{ ...panel }}>
        ...
        <div style={{ display: 'grid', gap: 8, marginBottom: 8 }}>
          ...
          <textarea
            style={{ ...inputStyle, height: 96, resize: 'vertical', whiteSpace: 'pre-wrap' }}
            placeholder={
