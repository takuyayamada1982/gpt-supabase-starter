...
export default function UPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      // ✅ プロファイル取得：id（= auth.uid）で1行だけ取る
      const { data: p, error: profileError } = await supabase
        .from('profiles')
        .select(
          'id, registered_at, trial_type, plan_status, plan_tier, is_canceled, plan_valid_until',
        )
        .eq('id', user.id) // ← email ではなく UUID で紐付け
        .maybeSingle();
